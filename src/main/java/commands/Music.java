...
public class Music implements Command {
    ...
    public void action(String[] args, MessageReceivedEvent event) {
        //System.out.print("play input registriert \n");
        ...
        switch (args[0].toLowerCase()) {
            ...
            case "louder":
                getPlayer(guild).setVolume(getPlayer(guild).getVolume()+100);
                break;

            case "volume": {
                System.out.print("xd\n");
                if (args.length < 2) {
                    event.getChannel().sendMessage("current Player Volume: `"+ String.valueOf(getPlayer(guild).getVolume())+"`").queue();
                    break;
                }
                else
                {
                    try
                    {
                        ...
                        int oldVolume = getPlayer(guild).getVolume();
                        getPlayer(guild).setVolume(newVolume);
